...
0 merged, most closed unmerged, several still open** (#83–#88 at last count). Recommend
...
until an owner ruling lands. Every run since (including PR #88 and this one) has had to
re-add this file from scratch because no prior status PR has ever been merged — see the
process note above and the process-note push notification sent by this run.
