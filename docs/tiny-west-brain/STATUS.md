...
### Operational issue — flagged again, still unresolved (owner action needed)

As of this run there are **35+ open, unmerged, draft "status check-in" PRs**
(#111–#145), all produced by this same scheduled prompt firing repeatedly (every
1–3 hours) since about 2026-07-22, plus roughly 40 more already closed unmerged.
**None have ever been merged.** Several PRs in the backlog ask to "merge #105" or
"merge #144" as the fix — those are themselves just more unmerged duplicates, not
a resolution. This entry does not change that pattern by itself; a human needs to
either pause/slow the schedule or merge one PR and close the rest (none carry
...
file removed. The owner should verify rotation status directly.
